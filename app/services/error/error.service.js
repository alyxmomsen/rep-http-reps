...
const {
    errorCodeHandlers: multipartErrorHandlers,
} = require('../request-handlers/form/controller/error.handlers');
...
class ErrorService {
    ...
    handleError(subjectId, errorCode, payload) {
        ...
        if (!subjectName) {
            throw new Error(
                `error-subject by id:${subjectId} is not registered`
            );
        }
...
errorService.addListener(
    GLOBAL_NAMES.MULTIPART_HANDLER,
    multipartErrorHandlers
);
