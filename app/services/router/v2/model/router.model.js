...
class HTTPRouter {
    ...
    async #executeMiddleware(req, res, middleware) {
        ...
        const next = async () => {
            if (index < middleware.length) {
                ...
                throw new Error(
                    `Router.executeMiddleware: no handler received`
                );
            }
        ...
        return;
        throw new Error(
            `router.executemiddleware: middleware.lenght must be > 0`
        );
    }
    ...
    #extractQueryParams(queryString) {
        ...
        if (
            (typeof queryString === 'string' || queryString === null) === false
        ) {
            throw new Error(
                `Router/#extractQueryParams: incorrect the arg value type`
            );
        }
    ...
    #addRoute(template, method, middleware) {
        if (
            Object.keys(LocalConstants.MethodsKeys).includes(method) === false
        ) {
        ...
        console.log(
            `successfully registrated new route: ${method} ${template}`
        );
    }
