...
const {
    defaultState: defaultStateDataSetFactory,
} = require('../../../../../__tests__/delopment/prematcher/utils/icoming-data-set.factory');
...
class MultiTableGrouppingAgent {
    ...
    async handleFormDataPartParsedData(data) {
         ...
         */
        const { columnName, dataType, groupId, tableId } =
            this.#multitableProtocolParser(name);
        ...
        if (filename || contentType) {
            ...
            if (!filename) {
                throw new Error(
                    `MultiTableGrouppingAgent: as "contentType" received then "filename" required too`
                );
            }

            if (!contentType) {
                throw new Error(
                    `MultiTableGrouppingAgent: as "filename" received then "contentType" required too`
                );
            }
    ...
    constructor(deps = {}) {
        ...
        const regularFieldDataSetSchema =
            deps.regularFieldDataSetSchema || null;
        const linkedFieldDataSetSchema = deps.linkedFieldDataSetSchema || null;

        /* DATA-SET FACTORIES */

        const fileDataSetFactory = deps.fileDataSetAdapter || null;
        const regularFieldDataSetFactory =
            deps.regularFieldDataSetAdapter || null;
        const linkedFieldDataSetFactory =
            deps.linkedFieldDataSetAdapter || null;
        ...
        console.log(
            `\x1b[32mMultiTableGrouppingAgent: ✔ dataTransformer plugged`
        );

        if (!multiTableProtocolParser) {
            throw new Error(
                `multiTableProtocolParser required but not provided`
            );
        }

        console.log(
            `\x1b[32mMultiTableGrouppingAgent: ✔ multiTableProtocolParser plugged`
        );
