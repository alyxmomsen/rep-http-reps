...
function parseFormDataPart(formDataPart) {
    const { body, headers: formDataPartHeadersRawData } =
    ...
    const contentDisposition =
        formDataPartHeaders['content-disposition'] || null;
    if (!contentDisposition) {
        throw new Error(`parseFormDataPart: no content-disposition header`);
    }
    const { name: name, filename: filename } =
...
function splitFormDataPart(formDataPart) {
    ...
    const separatorIndex = findSeparatorIndexInBuffer(
        formDataPart,
        separatorBuffer
    );
